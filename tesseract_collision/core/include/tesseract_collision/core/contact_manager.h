/**
 * @file _contact_manager.h
 * @brief This is the  contact manager base class
 *
 * It should be used to perform  contact checking.
 *
 ...
 */
#ifndef TESSERACT_COLLISION_CONTACT_MANAGER_H
#define TESSERACT_COLLISION_CONTACT_MANAGER_H
...
public:
  ...
  using Ptr = std::shared_ptr<ContactManager>;
  using ConstPtr = std::shared_ptr<const ContactManager>;
  using UPtr = std::unique_ptr<ContactManager>;
  using ConstUPtr = std::unique_ptr<const ContactManager>;
   ...
   */
  virtual ContactManager::UPtr clone() const = 0;
...
#endif  // TESSERACT_COLLISION_CONTACT_MANAGER_H
